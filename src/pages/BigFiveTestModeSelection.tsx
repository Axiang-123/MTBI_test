...
const testModes: TestMode[] = [
  {
    id: 'quick',
    name: '快速模式',
    description: '5-8分钟内完成，适合快速了解自己的性格倾向',
    duration: '5-8分钟',
    questions: 40,
    color: 'bg-green-500',
...
const BigFiveTestModeSelection: React.FC = () => {
  const navigate = useNavigate();
  const resetTest = useBigFiveTestStore((state) => state.resetTest);

  const handleModeSelect = (modeId: string) => {
    resetTest();
    navigate(`/big-five/test/${modeId}`);
  };
  ...
  return (
    <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8">
      ...
      >
        <h1 className="text-4xl font-extrabold text-gray-900 mb-4">大五人格测试</h1>
        <p className="text-xl text-gray-600 mb-12">选择适合您的测试模式，探索您的性格特征</p>
...
export default BigFiveTestModeSelection;